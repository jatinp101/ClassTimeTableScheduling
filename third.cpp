...
void third(map<int, map<int,int> >& classTable, vector<vector<int> >& schedule, vector<vector<int> >& schedule2, vector<int> crnTeacher, int tCost){
    ...
    double r = 0.77;//a factor to control the speed of cool down
    ...
    while(T > T_min){
        while(cur_swap < max_swap){
            ...
            int row_tem = rand() % 100;//time
            int col_tem = rand() % 100;//classroom
            ...
            if(tCost == 0) return;// no class clash anymore
